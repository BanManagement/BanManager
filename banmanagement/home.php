...
function latestBans($server, $serverID) {	
	...
	if($rows == 0)
		...
		foreach($result as $r) {
			...
			echo '
					<li class="latestban"><a href="index.php?action=viewplayer&player='.$r['banned'].'&server='.$serverID.'"><img src="http://cravatar.eu/helmavatar/'.$r['banned'].'/23" alt="'.$r['banned'].'" class="minihead" /> '.$r['banned'].'</a><button class="btn btn-info ban-info" rel="popover" data-html="true" data-content="'.$r['ban_reason'].'" data-original-title="'.$r['banned_by'];
...
function latestMutes($server, $serverID) {	
	...
	if($rows == 0)
		...
		foreach($result as $r) {
			...
			echo '<li class="latestban"><a href="index.php?action=viewplayer&player='.$r['muted'].'&server='.$serverID.'"><img src="http://cravatar.eu/helmavatar/'.$r['muted'].'/23" alt="'.$r['muted'].'" class="minihead" /> '.$r['muted'].'</a><button class="btn btn-info ban-info" rel="popover" data-html="true" data-content="'.$r['mute_reason'].'" data-original-title="'.$r['muted_by'];

...
public class TempBanCommandTest extends BasePluginDbTest {
  ...
  public void shouldFailIfAmbiguousPartialMatch() {
    ...
    CommonSender sender = spy(plugin.getServer().getConsoleSender());
    String[] args = new String[]{offlinePlayer.getName(), "1d", "test"};
  ...
  public void shouldAllowPartialWhenNoExactCollision() {
    ...
    CommonSender sender = spy(plugin.getServer().getConsoleSender());
    String[] args = new String[]{"Play", "1d", "test"};
  ...
  public void shouldFailIfOfflineExempt() {
    ...
    when(sender.hasPermission("bm.exempt.override.tempban")).thenReturn(false);
    when(commonPlayer.hasPermission("bm.exempt.tempban")).thenReturn(true);

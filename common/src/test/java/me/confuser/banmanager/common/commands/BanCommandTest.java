...
public class BanCommandTest extends BasePluginDbTest {
  ...
  public void shouldBlockSelfTargetThroughPartialName() {
    ...
    server.clearExactMatches();
    server.clearPartialMatches();
    server.setPartialMatch("Player", (CommonPlayer) sender);

    try {
      assert (cmd.onCommand(sender, new CommandParser(plugin, args, 1)));
      verify(sender).sendMessage("&cMultiple players match \"" + offlinePlayer.getName() + "\". Please use their full name.");
      assertFalse(plugin.getPlayerBanStorage().isBanned(self.getUUID()));
      assertFalse(plugin.getPlayerBanStorage().isBanned(offlinePlayer.getUUID()));
    } finally {
      server.clearPartialMatches();
      server.clearExactMatches();
      server.setUseStorageForOnlineLookups(true);
  ...
  public void shouldAllowPartialWhenNoExactCollision() {
    ...
    try {
      assert (cmd.onCommand(sender, new CommandParser(plugin, args, 1)));
      await().until(() -> plugin.getPlayerBanStorage().isBanned(targetPlayer.getUUID()));

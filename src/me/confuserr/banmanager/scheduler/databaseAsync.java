...
public class databaseAsync implements Runnable {
	...
	public void run() {
		...
		//System.out.println("INSERT INTO "+plugin.localBanRecordTable+" (banned, banned_by, ban_reason, ban_time, ban_expired_on, unbanned_by, unbanned_time, server) SELECT b.banned, b.banned_by, b.ban_reason, b.ban_time, b.ban_expires_on, 'Console automated', "+now+", b.server FROM "+plugin.localBansTable+" b WHERE b.ban_expires_on != '0' AND b.ban_expires_on < "+now+"");
		...
		try {
			while(result.next()) {
				...
				//plugin.getServer().getPlayer(result.getString("banned")).setBanned(false);
			}
			result.close();
		...
		if(plugin.keepKicks > 0) {
			int days = 86400 * plugin.keepKicks;

...
public class BmAPI {
	...
	private static List<BmTool> tools = new ArrayList<BmTool>();
	...
	public static void registerTool(BmTool tool) {
		if (tool.name != null) {
			// Check if tool name already exist
			if (getTool(tool.name) == null) {
				tools.add(tool);
			}
		}
	}

	public static List<BmTool> getTools() {
		return tools;
	}

	/**
	 * Get a BmTool from its name.
	 * Returns null if tool doesn't exist.
	 *
	 * @param name Tool name
	 * @return The tool
	 */
	public static BmTool getTool(String name) {
		for (BmTool toolToCheck : getTools()) {
			if (toolToCheck.name.equalsIgnoreCase(name))
				return toolToCheck;
		}
		return null;
	}

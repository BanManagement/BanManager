...
public class LoglessKickCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String commandLabel, String args[]) {
		...
		if(list.size() == 1) {
			...
			if(target.getName().equals(playerName)) {
			...
			} else if(!sender.hasPermission("bm.exempt.override.kick") && target.hasPermission("bm.exempt")) {
				...
				if(reason.isEmpty())
					message = plugin.getMessage("kickedNo").replace("[name]", target.getDisplayName()).replace("[by]", playerName);
				else
					message = plugin.getMessage("kicked").replace("[name]", target.getDisplayName()).replace("[reason]", viewReason).replace("[by]", playerName);
